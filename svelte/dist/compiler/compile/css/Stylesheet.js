...
class Rule {
    constructor(node, stylesheet, parent) {
        this.node = node;
        this.parent = parent;
        this.selectors = node.selector.children.map((node) => new Selector_1.default(node, stylesheet));
...
class Stylesheet {
    constructor(source, ast, filename, dev) {
        ...
        if (ast.css && ast.css.children.length) {
            this.id = `svelte-${hash_1.default(ast.css.content.styles)}`;
            this.has_styles = true;
            const stack = [];
            let depth = 0;
            let current_atrule = null;
            estree_walker_1.walk(ast.css, {
                enter: (node) => {
                    if (node.type === 'Atrule') {
    ...
    render(file, should_transform_selectors) {
        ...
        const code = new magic_string_1.default(this.source);
        estree_walker_1.walk(this.ast.css, {
            enter: (node) => {
