...
exports.BaseAttributeWrapper = void 0;
...
exports.BaseAttributeWrapper = BaseAttributeWrapper;
class AttributeWrapper extends BaseAttributeWrapper {
    constructor(parent, block, node) {
        ...
        if (node.dependencies.size > 0) {
            ...
            if (node.name === 'value') {
                handle_select_value_binding_1.default(this, node.dependencies);
            }
        }
        this.name = fix_attribute_casing_1.default(this.node.name);
    ...
    render(block) {
        ...
        if (is_legacy_input_type) {
            block.chunks.hydrate.push(code_red_1.b `@set_input_type(${element.var}, ${init});`);
            updater = code_red_1.b `@set_input_type(${element.var}, ${should_cache ? this.last : value});`;
        }
        else if (this.is_select_value_attribute) {
            // annoying special case
            const is_multiple_select = element.node.get_static_attribute_value('multiple');
            if (is_multiple_select) {
                updater = code_red_1.b `@select_options(${element.var}, ${value});`;
            }
            else {
                updater = code_red_1.b `@select_option(${element.var}, ${value});`;
            }
            ...
            block.chunks.hydrate.push(code_red_1.b `if (${element.var}.src !== ${init}) ${method}(${element.var}, "${name}", ${this.last});`);
            updater = code_red_1.b `${method}(${element.var}, "${name}", ${should_cache ? this.last : value});`;
        }
        else if (property_name) {
            block.chunks.hydrate.push(code_red_1.b `${element.var}.${property_name} = ${init};`);
            updater = block.renderer.options.dev
                ? code_red_1.b `@prop_dev(${element.var}, "${property_name}", ${should_cache ? this.last : value});`
                : code_red_1.b `${element.var}.${property_name} = ${should_cache ? this.last : value};`;
        }
        else {
            block.chunks.hydrate.push(code_red_1.b `${method}(${element.var}, "${name}", ${init});`);
            updater = code_red_1.b `${method}(${element.var}, "${name}", ${should_cache ? this.last : value});`;
        }
        if (is_indirectly_bound_value) {
            const update_value = code_red_1.b `${element.var}.value = ${element.var}.__value;`;
            block.chunks.hydrate.push(update_value);
            updater = code_red_1.b `
				${updater}
				${update_value};
			`;
        }
    ...
    get_init(block, value) {
        ...
        return this.should_cache ? code_red_1.x `${this.last} = ${value}` : value;
    }
    get_dom_update_conditions(block, dependency_condition) {
        ...
        if (should_cache) {
            condition = this.is_src
                ? code_red_1.x `${condition} && (${element.var}.src !== (${last} = ${value}))`
                : code_red_1.x `${condition} && (${last} !== (${last} = ${value}))`;
        }
        if (this.is_input_value) {
            const type = element.node.get_static_attribute_value('type');
            if (type === null || type === "" || type === "text" || type === "email" || type === "password") {
                condition = code_red_1.x `${condition} && ${element.var}.${property_name} !== ${should_cache ? last : value}`;
            }
        }
        if (block.has_outros) {
    ...
    get_value(block) {
        ...
        if (this.node.chunks.length === 1) {
            return this.node.chunks[0].type === 'Text'
                ? stringify_1.string_literal(this.node.chunks[0].data)
                : this.node.chunks[0].manipulate(block);
        }
        let value = this.node.name === 'class'
            ? this.get_class_name_text(block)
            : this.render_chunks(block).reduce((lhs, rhs) => code_red_1.x `${lhs} + ${rhs}`);
    ...
    get_class_name_text(block) {
        ...
        if (scoped_css && rendered.length === 2) {
            ...
            rendered[0] = code_red_1.x `@null_to_empty(${rendered[0]})`;
        }
        return rendered.reduce((lhs, rhs) => code_red_1.x `${lhs} + ${rhs}`);
    }
    render_chunks(block) {
...
exports.default = AttributeWrapper;
