...
const create_module_1 = __importDefault(require("./create_module"));
const scope_1 = require("./utils/scope");
const Stylesheet_1 = __importDefault(require("./css/Stylesheet"));
const config_1 = require("../config");
const Fragment_1 = __importDefault(require("./nodes/Fragment"));
const internal_exports_1 = __importDefault(require("./internal_exports"));
const error_1 = __importDefault(require("../utils/error"));
const get_code_frame_1 = __importDefault(require("../utils/get_code_frame"));
const flatten_reference_1 = __importDefault(require("./utils/flatten_reference"));
const is_used_as_reference_1 = __importDefault(require("./utils/is_used_as_reference"));
...
const check_graph_for_cycles_1 = __importDefault(require("./utils/check_graph_for_cycles"));
...
class Component {
    constructor(ast, source, name, compile_options, stats, warnings) {
            ...
            compile_options.filename &&
                (typeof process !== 'undefined'
                    ? compile_options.filename
                        ...
                        .replace(/^[/\\]/, '')
                    : compile_options.filename);
        this.locate = locate_character_1.getLocator(this.source, { offsetLine: 1 });
        // styles
        this.stylesheet = new Stylesheet_1.default(source, ast, compile_options.filename, compile_options.dev);
        this.stylesheet.validate(this);
        this.component_options = process_component_options(this, this.ast.html.children);
        this.namespace =
            namespaces_1.namespaces[this.component_options.namespace] ||
        ...
        this.fragment = new Fragment_1.default(this, ast.html);
    ...
    add_reference(name) {
        ...
        else if (reserved_keywords_1.is_reserved_keyword(name)) {
            this.add_var({
                name,
                injected: true,
                referenced: true
    ...
    generate(result) {
        ...
        if (result) {
            ...
            estree_walker_1.walk(program, {
                enter: (node, parent, key) => {
                    if (node.type === 'Identifier') {
                        if (node.name[0] === '@') {
                            if (node.name[1] === '_') {
                                ...
                                if (compile_options.dev) {
                                    if (internal_exports_1.default.has(`${name}_dev`)) {
                                        name += '_dev';
                                    }
                                    else if (internal_exports_1.default.has(`${name}Dev`)) {
                                        name += 'Dev';
                                    }
                                }
                                const alias = this.alias(name);
                        ...
                        else if (node.name[0] !== '#' && !names_1.is_valid(node.name)) {
            ...
            create_module_1.default(program, format, name, banner, compile_options.sveltePath, imported_helpers, referenced_globals, this.imports, this.vars
    ...
    get_unique_name(name, scope) {
        if (config_1.test)
            name = `${name}$`;
        let alias = name;
        for (let i = 1; names_1.reserved.has(alias) ||
    ...
    get_unique_name_maker() {
        ...
        names_1.reserved.forEach(add);
        internal_exports_1.default.forEach(add);
    ...
    warn(pos, warning) {
        ...
        const frame = get_code_frame_1.default(this.source, start.line - 1, start.column);
        this.warnings.push({
            code: warning.code,
            ...
            if (node.declaration) {
                if (node.declaration.type === 'VariableDeclaration') {
                    node.declaration.declarations.forEach(declarator => {
                        scope_1.extract_names(declarator.id).forEach(name => {
                            const variable = this.var_lookup.get(name);
    ...
    walk_module_js() {
        ...
        if (!script)
            return;
        estree_walker_1.walk(script.content, {
            enter(node) {
        ...
        const { scope, globals } = scope_1.create_scopes(script.content);
    ...
    walk_instance_js_pre_template() {
        ...
        script.content.body.forEach(node => {
            ...
            if (expression.left.type === 'MemberExpression')
                return;
            scope_1.extract_names(expression.left).forEach(name => {
                if (!this.var_lookup.has(name) && name[0] !== '$') {
                    this.injected_reactive_declaration_vars.add(name);
                }
            });
        });
        const { scope: instance_scope, map, globals } = scope_1.create_scopes(script.content);
    ...
    post_template_walk() {
        ...
        if (scope_updated) {
            const { scope, map } = scope_1.create_scopes(script.content);
        ...
        estree_walker_1.walk(content, {
            enter(node, parent) {
                ...
                if (node.type === 'AssignmentExpression' || node.type === 'UpdateExpression') {
                    ...
                    const names = scope_1.extract_names(assignee);
                ...
                if (is_used_as_reference_1.default(node, parent)) {
                    const object = get_object_1.default(node);
    ...
    loop_protect(node, scope, timeout) {
        if (node.type === 'WhileStatement' ||
            ...
            const before = code_red_1.b `const ${guard} = @loop_guard(${timeout})`;
            const inside = code_red_1.b `${guard}();`;
    ...
    rewrite_props(get_insert) {
        ...
        const { instance_scope, instance_scope_map: map } = this;
        let scope = instance_scope;
        estree_walker_1.walk(this.ast.instance.content, {
                    ...
                    if (node.kind === 'var' || scope === instance_scope) {
                        node.declarations.forEach(declarator => {
                            ...
                            if (variable.export_name && variable.writable) {
                                ...
                                declarator.init = code_red_1.x `$$props`;
                            }
    ...
    hoist_instance_declarations() {
        ...
        const is_hoistable = fn_declaration => {
            ...
            estree_walker_1.walk(fn_declaration, {
                enter(node, parent) {
                    if (!hoistable)
                        return this.skip();
                    if (map.has(node)) {
    ...
    extract_reactive_declarations() {
        ...
        this.ast.instance.content.body.forEach(node => {
            if (node.type === 'LabeledStatement' && node.label.name === '$') {
                ...
                let scope = this.instance_scope;
                const map = this.instance_scope_map;
                estree_walker_1.walk(node.body, {
                    enter(node, parent) {
                        if (map.has(node)) {
                            scope = map.get(node);
                        }
                        if (node.type === 'AssignmentExpression') {
                            const left = get_object_1.default(node.left);
                            scope_1.extract_identifiers(left).forEach(node => {
                            ...
                            const identifier = get_object_1.default(node.argument);
                            assignees.add(identifier.name);
                        }
                        else if (is_reference_1.default(node, parent)) {
                            const identifier = get_object_1.default(node);
        ...
        const cycle = check_graph_for_cycles_1.default(unsorted_reactive_declarations.reduce((acc, declaration) => {
    ...
    warn_if_undefined(name, node, template_scope) {
        if (name[0] === '$') {
            if (name === '$' || name[1] === '$' && !reserved_keywords_1.is_reserved_keyword(name)) {
                this.error(node, {
                    code: 'illegal-global',
                    message: `${name} is an illegal variable name`
                });
        ...
        if (template_scope && template_scope.names.has(name))
            return;
        if (names_1.globals.has(name) && node.type !== 'InlineComponent')
            return;
    ...
    push_ignores(ignores) {
        ...
        add_to_set_1.default(this.ignores, ignores);
            ...
            if (attribute.type === 'Attribute') {
                ...
                switch (name) {
                    ...
                    case 'namespace': {
                        ...
                        if (typeof ns !== 'string')
                            component.error(attribute, { code, message });
                        if (namespaces_1.valid_namespaces.indexOf(ns) === -1) {
                            const match = fuzzymatch_1.default(ns, namespaces_1.valid_namespaces);
                            if (match) {
                                component.error(attribute, {
